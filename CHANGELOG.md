...
- **A few tasks can also become reminders.** Marked while planning, they go into a queue in the plan; a helper, the app or a Shortcut on an Apple device applies it, and iCloud carries the result to every device. The plugin itself never writes to Reminders, and a reminder ticked there ticks its task in the note. They go to the same list as the Erinnerungen sync, and a task the sync already keeps is left to it, so no task is reminded twice.
