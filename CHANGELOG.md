...
- **Any file can be given a name in the pane.** Obsidian's properties are Markdown and nothing else: a PDF, an image, a spreadsheet or a folder carries no metadata at all, so the filename is the whole searchable surface — and in a vault of scans that filename is `2024-03-14_scan_0042.pdf`. **Name in the pane…** on the context menu gives any file a line to be drawn by, and the row shows it everywhere the file appears, in the tree as well as in Pinned and Latest. The file itself is never touched: it keeps its name, links to it keep working, and the row's tooltip still carries the full path, so where the file lives is never further away than a hover. The filter searches the given name alongside the filename, so a file can be found by either, and a folder whose contents match still opens. The name moves with the file, comes back if it reappears elsewhere within thirty days, and merges per file between devices, because it is held in the same place the icons are. Where it is not: Obsidian's own search, Bases and graph cannot see it — it is the plugin's, not the vault's.
