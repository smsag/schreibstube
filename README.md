...
- **Naming a file in the pane.** Obsidian's properties are Markdown and nothing else, so a PDF, an image or a spreadsheet has no metadata at all and can only be found by its filename — which in a vault of scans is usually `2024-03-14_scan_0042.pdf`. **Name in the pane…** gives any file, folders included, a line to be drawn by instead. The file is never touched: it keeps its name, links to it keep working, and the row's tooltip still carries the full path. The name is searched by the filter alongside the filename, so a file can be found by either. Where you see it: on the row, in every section of the pane, and in `explorer.json` — it is held by the plugin, so Obsidian's own search, Bases and graph do not know about it.
