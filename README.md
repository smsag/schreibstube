...
A writing-focused Obsidian plugin: a sticky heading-stack overlay, a distraction-reducing focus mode, LLM-powered file renaming, side-pane link opening, and a task summary ribbon with per-heading counts.
...
- **Insert task summary ribbon** — inserts a ```` ```schreibstube-tasks ```` block at the cursor. The block renders as a one-line ribbon, e.g. **20** open of **21**, counting every task in the note.
