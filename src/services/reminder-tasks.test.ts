...
describe("editing a task line", () => {
  it("ticks and reopens the box, and leaves a box someone marked otherwise alone", () => {
    expect(withDone("- [ ] x ^r-a", true)).toBe("- [x] x ^r-a");
    expect(withDone("  1. [x] x", false)).toBe("  1. [ ] x");
    expect(withDone("- [-] cancelled", true)).toBe("- [-] cancelled");
    expect(withDone("- [-] cancelled", false)).toBe("- [-] cancelled");
  });
...
describe("a task the sync already keeps", () => {
  const task = (line: string) => tasksInNote("Plan.md", line)[0]!;

  it("is one with the tag, or with a date while dates count", () => {
    expect(syncedAsReminder(task("- [ ] Call #remind"), "tag")).toBe(true);
    expect(syncedAsReminder(task("- [ ] Call 📅 2026-09-20"), "date")).toBe(true);
    expect(syncedAsReminder(task("- [ ] Call 📅 2026-09-20"), "tag")).toBe(false);
    expect(syncedAsReminder(task("- [ ] Call #projects/ea48"), "date")).toBe(false);
  });
});
