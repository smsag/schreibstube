...
const BLOCK_ID_PATTERN = BLOCK_ID;
/** Read once to migrate it: see `LEGACY_REMINDER_LINK`. */
const LEGACY_LINK_PATTERN = LEGACY_REMINDER_LINK;
...
/**
 * Whether the Erinnerungen sync already keeps this task as a reminder.
 *
 * The planner can mark a task for Reminders too. When the sync carries it
 * already — it has the tag, or a due date and dates count — the planner
 * leaves it to the sync, so one task is one reminder, not one per feature.
 */
export function syncedAsReminder(task: VaultTask, trigger: ReminderTrigger): boolean {
  return task.tags.includes("remind") || (trigger === "date" && task.due !== null);
}
...
export function taskTitle(line: string): string {
  const text = taskText(line).replace(REMIND_TAG_PATTERN, "$1").replace(/\s+/g, " ").trim();
  return text.length > MAX_REMINDER_TITLE_CHARS
    ? `${text.slice(0, MAX_REMINDER_TITLE_CHARS - 1)}…`
    : text;
}

/** The line with its box ticked or reopened, by the one rule for a box. */
export function withDone(line: string, done: boolean): string {
  return withBoxDone(line, done);
}
